...
public class CreateFacilitiesGAP_MID_Data {
	...
	private final static Logger log = Logger.getLogger(CreateFacilitiesGAP_MID_Data.class);
	...
	private CoordinateTransformation ct = TransformationFactory.getCoordinateTransformation("EPSG:4326", "EPSG:32632");
	/*
	 * stores the capacity of workers for workplaces, i.e. for facilities with type "work", and 
	 * corresponding facilityId.
	 */
	private Map< Id, Integer> facilityCapacities = new HashMap<Id, Integer>();
	/*
	 * stores two types of Ids, personId and the corresponding facilityId for the persons home-facility.
	 */
	...
	public static void main(String[] args) {
		...
		facilitiesCreator.scenario = ScenarioUtils.createScenario(config);			facilitiesCreator.run();
	...
	public void run() {
		...
		/*
		 * Read the person census and create home facilities.
		 */
	...
	private int readBusinessCensus() {
		...
		try {
			...
			// Speichere eine ausgelesene Zeile in parts[]
			while ((line = bufferedReader.readLine()) != null) {
				String parts[] = line.split("\t");
				
				Coord businessCoord = new CoordImpl(Double.parseDouble(parts[index_xCoord]),
				...
				// Falls zu einer facilityID mehrere activityTypes gehören, speichere diese in types[]
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if(! ageString.equals("NULL")){
				  ...
				  Coord homeCoordinate = CoordsHomeFac.CreateRandomCoordinateInMunicipality(municipal) ;
				  Id facilityId = new IdImpl(startIndex+cnt);
				
				  ActivityFacility facility = ((ScenarioImpl)this.scenario).getActivityFacilities().createAndAddFacility(facilityId, homeCoordinate);
	...
	public void write() {
		new FacilitiesWriter(((ScenarioImpl) this.scenario).getActivityFacilities()).write("./input/facilitiesGAP_mitHome.xml");  // facilitiesGAP.xml.gz
	}
}
